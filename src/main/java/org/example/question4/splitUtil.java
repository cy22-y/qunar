...
public class splitUtil {
    ...
    //返回主命令
    public static String command(String msg){
        msg = msg.trim();
    ...
    //分割得到filename
    public static String filename(String msg){
        msg = msg.trim();

        String[] res = msg.split(" ");
        if(res[0].equals(Commands.CAT_COMMAND))
        {
            System.out.println(res[1]);
            return res[1];
        }
        else if(res[0].equals(Commands.GREP_COMMAND)){
            return res[2];
        }else if(res[0].equals(Commands.WC_COMMAND)){
